...
This document outlines the structural refactoring required to implement a Wireshark-style "Workspace Hub" (Launchpad). The goal is to eliminate the clunky "Unified Interface" and replace it with a clean starting page that routes users to specialized application shells.

## Proposed Changes

### Routing & State Management

Currently, `App.tsx` forces the user into `#/app` and renders the monolithic `AppLayout`. We will replace this with a hash-based router.
