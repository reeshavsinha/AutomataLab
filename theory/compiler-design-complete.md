...
3. [Module 3: Bottom-Up Parsing](#module-3-bottom-up-parsing)
   ...
   - [3.2 The LR Parsing Family: Core Theory & LR(0) Automata](#32-the-lr-parsing-family-core-theory--lr0-automata)
   - [3.3 SLR(1), CLR(1), and LALR(1) Parsers](#33-slr1-clr1-and-lalr1-parsers)
   - [3.4 LR Parsing Table Construction: ACTION & GOTO Tables](#34-lr-parsing-table-construction-action--goto-tables)
   - [3.5 Shift-Reduce & Reduce-Reduce Conflict Analysis](#35-shift-reduce--reduce-reduce-conflict-analysis)
   - [3.6 Parsing Power & State Complexity Comparison](#36-parsing-power--state-complexity-comparison)
...
A grammar $G$ is **ambiguous** if there exists a string $w \in L(G)$ that can produce two or more distinct parse trees (equivalent to having multiple distinct leftmost or rightmost derivations) [185, 762].

Consider the classic expression grammar [185]:
...
#### 2. Left Factoring
If a variable has multiple production choices starting with a common prefix (e.g., $A \to \alpha\beta_1 \mid \alpha\beta_2$), the parser cannot decide which production to select with $1$ lookahead symbol [214]. We apply left factoring to defer the choice [214]:
$$\begin{aligned}
A &\to \alpha A' \\
...
### 3.2 The LR Parsing Family: Core Theory & LR(0) Automata
...
### 3.3 SLR(1), CLR(1), and LALR(1) Parsers
...
### 3.4 LR Parsing Table Construction: ACTION & GOTO Tables
...
### 3.5 Shift-Reduce & Reduce-Reduce Conflict Analysis
...
### 3.6 Parsing Power & State Complexity Comparison
The computational power of parsers follows a strict hierarchy of nested classes:
$$LL(1) \subset SLR(1) \subset LALR(1) \subset CLR(1)$$

Below is a state complexity and parsing capability comparison matrix:

| Parser Variant | State Count Complexity | Lookahead Logic | Resolves Shift-Reduce Conflicts? | Resolves Reduce-Reduce Conflicts? |
| :--- | :--- | :--- | :--- | :--- |
| **LR(0)** | Minimal ($N$ states) | None ($0$ lookahead) | No | No |
| **SLR(1)** | Minimal ($N$ states) | $\text{FOLLOW}(A)$ of LHS | Partially | Partially |
| **LALR(1)** | Minimal ($N$ states) | Propagated lookahead | Yes | Yes (most cases) |
| **CLR(1)** | Exponential ($10 \times N$) | Explicit LR(1) lookahead | Yes | Yes (all cases) |

---
## Module 4: Syntax-Directed Translation

### 4.1 Syntax-Directed Definitions (SDDs): Attributes & Rules
A **Syntax-Directed Definition (SDD)** is a context-free grammar coupled with **attributes** and **semantic rules** [395].
*   **Attributes**: Quantities associated with grammar symbols (e.g., numeric values, types, table offsets, target code strings) [228, 395].
*   **Semantic Rules**: Mathematical operations associated with grammar productions that define the values of those attributes [395]. If production is $A \to \alpha$, the rule defines attributes for the head $A$ or the symbols in the body $\alpha$ [395].

### 4.2 Synthesized Attributes & S-Attributed Definitions
*   **Synthesized Attributes**: An attribute is synthesized if its value at a parse-tree node is computed strictly from the attribute values of its *children* [453].
*   **S-Attributed Definitions**: An SDD is S-attributed if every attribute in the grammar is synthesized [101, 312]. S-attributed definitions are highly desirable because they can be evaluated bottom-up during parsing without explicitly building the parse tree [101, 312, 417]. The semantic actions are executed at each reduction step [417].

```text
               Parse-Tree Node [ A.val = B.val + C.val ]
                                 ▲
                                 │ (Synthesized flow)
                  ┌──────────────┴──────────────┐
                  │                             │
          Child Node [ B.val ]          Child Node [ C.val ]
```

### 4.3 Inherited Attributes & L-Attributed Definitions
*   **Inherited Attributes**: An attribute is inherited if its value at a parse-tree node is computed from the attributes of its *parent* and/or its *siblings* [453].
*   **L-Attributed Definitions**: An SDD is L-attributed if its attributes are either synthesized or inherited, with the constraint that any inherited attribute of a symbol $X_j$ in the production $A \to X_1 X_2 \dots X_k$ can only depend on [101, 313, 402]:
    1. The inherited attributes of the parent $A$ [402].
    2. The attributes of sibling symbols $X_1 \dots X_{j-1}$ appearing to the *left* of $X_j$ [402].

This left-to-right evaluation order allows L-attributed definitions to be evaluated in a single pass during top-down predictive parsing [442, 454].
